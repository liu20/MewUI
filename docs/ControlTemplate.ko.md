...
1. **파트는 `OnApplyTemplate`부터 존재합니다.** `Template` 대입은 빌드를 예약할 뿐이고
   (다음 measure에서 lazy 빌드), 생성자나 대입 직후의 `GetTemplateChild`는 null입니다.
2. **`Build`는 컨트롤이 트리에 붙고 스타일이 해석된 뒤에 불립니다.** 그래서 `Build` 안에서
   테마/DPI 값을 읽어 사용해도 안전합니다. 테마가 바뀌면 새로 빌드되므로 값을 구워 써도
   됩니다.
3. **교체와 테마 전환은 트리의 소멸 이벤트입니다.** 이전 트리는 detach되고(내부 포커스는
   안전하게 해제) 다음 measure에서 새 트리가 빌드됩니다. 파트의 일시 상태(입력 중 텍스트,
   스크롤 위치 등)는 이때 사라지므로, **유지가 필요한 상태는 파트가 아니라 컨트롤의
   속성(logical)에 있어야 합니다.**

적용 후에는 measure, arrange, render, hit test, 트리 순회가 전부 **템플릿 루트 하나**를
통해 흐르고, 컨트롤의 기존 자체 시각 구현은 실행되지 않습니다.
